...
async function markEventInProgress(req, res) {
    try {
        ...
        if (event.isApproved && event.approvedEventStatus === "Scheduled") {
            ...
            res.status(400).json({
                error: "Event cannot be marked as In Progress"
            });
        }
    } catch (error) {
...
async function markEventCompleted(req, res) {
    try {
        ...
        if (event.isApproved && event.approvedEventStatus === "In Progress") {
            ...
            res.status(400).json({
                error: "Event cannot be marked as Completed"
            });
        }
    } catch (error) {
...
async function markEventPostponed(req, res) {
    try {
        ...
        if (event.isApproved && event.approvedEventStatus === "Scheduled") {
            ...
            await eventService.sendInterruptionEmail(
                event.invitedEmails,
                event
            );
            res.status(200).json({ event });
            ...
            res.status(400).json({
                error: "Event cannot be marked as postponed"
            });
        }
    } catch (error) {
...
async function markEventCancelled(req, res) {
    try {
        ...
        if (event.isApproved && event.approvedEventStatus === "Scheduled") {
            ...
            res.status(400).json({
                error: "Event cannot be marked as cancelled"
            });
        }
    } catch (error) {
...
async function viewEvent(req, res) {
    try {
        ...
        event.dataValues.invitedEmails = JSON.parse(
            event.dataValues.invitedEmails
        );
